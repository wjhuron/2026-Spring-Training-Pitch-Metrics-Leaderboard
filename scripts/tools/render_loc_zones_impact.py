...
Implements the four-part redesign:
  ...
  4. Cell grades are shrunk toward the league cell grade with k = 8 before
     ...
     (scripts/research/locplus/loczone_impact_shrink_sweep.py, interior optimum bracketed:
     r 0.3137 at k=0, 0.3169 at k=8, 0.3109 at k=110; flat 5-12).

Season pages only (prototype). Usage: python3 scripts/tools/render_loc_zones_impact.py
...
SHRINK_K = 8          # swept: scripts/research/locplus/loczone_impact_shrink_sweep.py
...
def main():
    ...
    if NEW_TAB_ARMS & set(PITCHERS):
        import gspread
        from pipeline.locplus import (compute_loc_plus, group_of, LOC_SCALE_K,
        ...
        from pipeline.utils import compute_in_zone
        for nm, plist in new_rows.items():
            kept = []
            for p in plist:
    ...
    for name, all_pitches in sorted(by_pitcher.items()):
      ...
      for hand, suffix, hand_label in PAGES:
        ...
        # scripts/research/locplus/loczone_callout_sweep.py -- realized-spread optimum at
