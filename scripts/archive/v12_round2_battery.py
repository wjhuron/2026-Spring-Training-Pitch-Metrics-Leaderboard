"""v12_round2_battery.py — AXIS + SPINEFF at the FULL production config.

Production build_df now emits axis_dev / axis_dev_abs / spin_eff; the
2021-24 pickles carry SpinAxis (augment_priors_spinaxis.py); 2025 prior has
axis_dev=None (no positional alignment path) but full spin_eff; 2026 sheets
carry RTilt/OTilt natively. Depth-7 TUNED, 2021-25 prior in every fold,
2026 pitcher-grouped 8-fold OOF.

Variants: BASE / AXIS(+axis_dev,axis_dev_abs) / SPINEFF(+spin_eff) / BOTH.

Usage: python3 scripts/v12_round2_battery.py
"""
...
print(f'2026 {len(df26)} rows; prior {len(prior)} rows; '
      f'prior axis_dev coverage {prior.axis_dev.notna().mean()*100:.0f}%, '
      f'spin_eff {prior.spin_eff.notna().mean()*100:.0f}%; '
      f'2026 axis {df26.axis_dev.notna().mean()*100:.0f}%, '
      f'spin_eff {df26.spin_eff.notna().mean()*100:.0f}%', flush=True)
...
VARIANTS = [
    ('BASE',    BASE),
    ('AXIS',    BASE + ['axis_dev', 'axis_dev_abs']),
    ('SPINEFF', BASE + ['spin_eff']),
    ('BOTH',    BASE + ['axis_dev', 'axis_dev_abs', 'spin_eff']),
]
for name, feats in VARIANTS:
    ...
    desc = -pear(dx, dy)
    # drain check: unit resid vs axis_dev_abs / spin_eff
    d['resid'] = d['target_xrv'] + d['stuff']
    g = d.groupby(['pitcher', 'throws', 'pitch_type']).agg(
        resid=('resid', 'mean'), n=('resid', 'size'),
        adev=('axis_dev_abs', 'mean'), seff=('spin_eff', 'mean'))
    gq = g[g['n'] >= 100].dropna()
    ra = pear(gq['resid'], gq['adev']) if len(gq) > 50 else float('nan')
    rs = pear(gq['resid'], gq['seff']) if len(gq) > 50 else float('nan')
    print(f'{name:8s} reliab {rel:.4f}  pred {pred:.4f}  desc {desc:.4f}  '
          f'resid~|axis| {ra:+.3f}  resid~spineff {rs:+.3f}  [{time.time()-t0:.0f}s]', flush=True)
