...
Usage:  python3 scripts/platoon_splits.py [--outdir ~/Downloads] [--no-new-tab]
"""
...
from pipeline_compute import (                                  # noqa: E402
    compute_stats, compute_expected_stats, compute_hitter_stats,
    compute_pitcher_batted_ball, compute_xrv, build_bip_count_means,
)
import pipeline_sdplus as SD                                    # noqa: E402
import pipeline_contact as CT                                   # noqa: E402
from pipeline_xwoba3d import classify_bip as xw3d_classify      # noqa: E402
...
def preprocess(all_pitches, new_rows, metadata, woba_weights):
    ...
    from pipeline_utils import compute_runexp_scale
    ...
    from pipeline_utils import BB_EVENTS, HBP_EVENTS, K_EVENTS
    n_fill = 0
    for p in all_pitches:
    ...
    for display, sheet_name, role, tab in TARGETS:
        ...
        for level, ps_all, sd_all in levels:
            if len(ps_all) < MIN_PITCHES_FOR_ROW:
                continue
            from pipeline_utils import NON_PA_EVENTS as _NPE
