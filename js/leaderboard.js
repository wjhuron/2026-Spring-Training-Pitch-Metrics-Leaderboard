const COLUMNS = {
  ...
  pitcherStats: [
    ...
    { key: 'commandPlus', label: 'Command+', format: Utils.formatInt, sortType: 'numeric', desc: "Command+ — execution repeatability: average miss distance from the pitcher's inferred targets (fit per pitch type, batter hand, and count situation). 100 = league avg, +10 = 1 SD tighter. Forecasts future walk rate beyond current BB%; it does NOT predict run prevention beyond Loc+ (command's run impact already lives in Loc+), and higher velocity trades against it. ROC pitchers scored against their own targets — no MLB translation.", group: 'run_value' },
    { key: 'pitchingScore', label: 'Pitching+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitching+ — overall arsenal quality: exactly 0.8 x Stuff+ + 0.2 x Loc+ (weights validated on future run prevention). Auditable from the two component columns.', group: 'run_value' },
  ...
  hitterStats: [
    ...
    { key: 'xWRCplus',    label: 'xWRC+',    format: Utils.formatInt, sortType: 'numeric', desc: 'wRC+ computed from expected wOBA instead of results — same scale, judges contact quality rather than outcomes', group: 'expected' },
  ...
  hitterBattedBall: [
    ...
    { key: 'bbPlus',     label: 'BB+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Batted-ball contact quality: xwOBAcon indexed so 100 = league avg, regressed to league by sample (n0=60 BIP). Scaled to wRC+\'s spread, so read it like wRC+: 120 is as far above average as a 120 wRC+.', group: 'expected' },
