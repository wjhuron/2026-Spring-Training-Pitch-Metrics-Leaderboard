const COLUMNS = {
  pitchMetrics: [
    ...
    { key: 'pitchingScore', label: 'Pitching+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitching+ for this pitch type — overall pitch quality: exactly 0.8 x Stuff+ + 0.2 x Loc+ (weights validated on future run prevention). Auditable from the two component columns.', group: 'outcomes' },
  ...
  pitcherStats: [
    ...
    { key: 'pitchingScore', label: 'Pitching+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitching+ — overall arsenal quality: exactly 0.8 x Stuff+ + 0.2 x Loc+ (weights validated on future run prevention). Auditable from the two component columns. It inherits their compression, so 1 SD is about 6 points — do not read it on the same 10-point ruler as Command+.', group: 'run_value' },
    { key: 'pitcherPlus', label: 'Pitcher+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitcher+ — the all-encompassing pitcher grade: stuff, command, whiffs, and results blended into one number. 100 = league avg, +10 = 1 SD better.', group: 'run_value' },
  ...
  hitterStats: [
    ...
    { key: 'xWRCplus',    label: 'xWRC+',    format: Utils.formatInt, sortType: 'numeric', desc: 'wRC+ computed from expected wOBA instead of results — same scale, judges contact quality rather than outcomes. As an estimate it prints slightly wider than wRC+ itself; the excess width is input sampling noise, so extreme values run a touch hotter/colder than the production they imply.', group: 'expected' },
    { key: 'hitterPlus',  label: 'Hitter+',  format: Utils.formatInt, sortType: 'numeric', desc: 'Hitter composite: weighted blend of BB+ (contact quality), SD+ (swing decisions), CT+ (contact rate). 100 = league avg, scaled to wRC+\'s spread — a gap vs wRC+ reads as process ahead of/behind results. Scale note: this is a skill index, not a production claim — measured agreement with same-season wRC+ is r about .77, so one displayed point carries about 0.8 points of wRC+ (a 140 Hitter+ implies roughly a 133 wRC+ process, not 140).', group: 'expected' },
  ...
  hitterBattedBall: [
    ...
    { key: 'bbPlus',     label: 'BB+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Batted-ball contact quality: xwOBAcon indexed so 100 = league avg, regressed to league by sample (n0=60 BIP). Scaled to wRC+\'s spread, so 120 is as RARE as a 120 wRC+ — rarity, not run value: measured, one SD of BB+ carries about 16 points of same-season wRC+ (2021-2026), because contact quality is one input to production, not all of it.', group: 'expected' },
  ...
  hitterSwingDecisions: [
    ...
    { key: 'nSwings',     label: 'Swings',   format: Utils.formatInt, sortType: 'numeric', noPercentile: true, group: 'info' },
    { key: 'pitchType',   label: 'Pitch',    format: function(v){ return v || ''; }, sortType: 'string', align: 'center', noPercentile: true, group: 'info', isPitchType: true },
    { key: 'sdPlus',      label: 'SD+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Swing-Decisions+ — did he swing at the right pitches? Every swing/take graded by the run value of that choice for its location and count, umpire-independent via the true zone. 100 = league avg, scaled to wRC+\'s spread so a 120 SD+ is as RARE as a 120 wRC+ — rarity, not run value: measured 2021-2026, one SD of swing decisions carries about 6 points of wRC+ (stable 2021-2026, both same-season and next-season). Read it as a skill grade (how good are the decisions), never as a production percentage.', group: 'discipline' },
    { key: 'ctPlus',      label: 'CT+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Contact+: leverage-weighted contact rate on swings. Frequency only — contact quality lives in BB+. 100 = league avg, scaled to wRC+\'s spread so a 120 CT+ is as RARE as a 120 wRC+ — rarity, not run value: measured, contact frequency alone carries roughly ZERO same-season wRC+ (quantity trades against quality). A real, stable skill; its production payoff arrives only in combination, via the Hitter+ weights.', group: 'discipline' },
