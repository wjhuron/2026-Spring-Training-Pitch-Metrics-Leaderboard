const COLUMNS = {
  pitchMetrics: [
    ...
    { key: 'pitchingScore', label: 'Pitching+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitching+ for this pitch type — overall pitch quality: exactly 0.8 x Stuff+ + 0.2 x Loc+ (weights validated on future run prevention). Auditable from the two component columns. Standard-deviation ruler, NOT a percent: a point is not 1% better. Hitter + metrics are percents; the pitcher ones are not, because Stuff+/Loc+/Pitching+ sit on the shared public convention so a value here means what it means on other sites.', group: 'outcomes' },
  ...
  pitcherStats: [
    ...
    { key: 'pitchingScore', label: 'Pitching+', format: Utils.formatInt, sortType: 'numeric', desc: 'Pitching+ — overall arsenal quality: exactly 0.8 x Stuff+ + 0.2 x Loc+ (weights validated on future run prevention). Auditable from the two component columns. It inherits their compression, so 1 SD is about 6 points. Standard-deviation ruler, NOT a percent: a point is not 1% better. Hitter + metrics are percents; the pitcher ones are not, because Stuff+/Loc+/Pitching+ sit on the shared public convention so a value here means what it means on other sites.', group: 'run_value' },
  ...
  hitterBattedBall: [
    ...
    { key: 'bbPlus',     label: 'BB+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Batted-ball contact quality: a blend of xwOBAcon (30%) and 95th-percentile exit velocity (70%), each regressed to league by sample before blending (n0 200 BIP for xwOBAcon, 0 for EV95 — a hard-hit ceiling barely needs regressing). Indexed so 100 = league avg; one point is one percent. EV95 carries the larger raw weight because its spread is much narrower, so it is about 28% of the actual variance.', group: 'expected' },
