const COLUMNS = {
  ...
  hitterStats: [
    ...
    { key: 'hitterPlus',  label: 'Hitter+',  format: Utils.formatInt, sortType: 'numeric', desc: 'Hitter composite: weighted blend of BB+ (contact quality), SD+ (swing decisions), CT+ (contact rate). 100 = league avg, printed at 0.82 x wRC+\'s spread — its measured agreement with wRC+ across 2021-2026 — so a displayed point IS an honest wRC+ point of process, and a gap vs wRC+ reads directly as process ahead of/behind results.', group: 'expected' },
  ...
  hitterBattedBall: [
    ...
    { key: 'bbPlus',     label: 'BB+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Batted-ball contact quality: xwOBAcon indexed so 100 = league avg, regressed to league by sample (n0=60 BIP). Printed at 0.66 x wRC+\'s spread — its measured share of same-season production (2021-2026) — so displayed points read as honest wRC+ points of contact-quality value.', group: 'expected' },
  ...
  hitterSwingDecisions: [
    ...
    { key: 'nSwings',     label: 'Swings',   format: Utils.formatInt, sortType: 'numeric', noPercentile: true, group: 'info' },
    { key: 'pitchType',   label: 'Pitch',    format: function(v){ return v || ''; }, sortType: 'string', align: 'center', noPercentile: true, group: 'info', isPitchType: true },
    { key: 'sdPlus',      label: 'SD+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Swing-Decisions+ — did he swing at the right pitches? Every swing/take graded by the run value of that choice for its location and count, umpire-independent via the true zone. 100 = league avg, +10 = 1 SD better decisions (the Command+ convention — an explicit skill ruler, no run claim in the scale). Measured 2021-2026: one SD of swing decisions carries about 6 wRC+ points, stable both same-season and next-season — a real skill with a modest run payoff, which is why it is not printed in run currency.', group: 'discipline' },
    { key: 'ctPlus',      label: 'CT+',      format: Utils.formatInt, sortType: 'numeric', desc: 'Contact+: leverage-weighted contact rate on swings. Frequency only — contact quality lives in BB+. 100 = league avg, +10 = 1 SD better contact frequency (the Command+ convention — an explicit skill ruler, no run claim in the scale). Measured 2021-2026: frequency alone carries roughly ZERO wRC+ (quantity trades against quality); its production payoff arrives only in combination, via the Hitter+ weights.', group: 'discipline' },
