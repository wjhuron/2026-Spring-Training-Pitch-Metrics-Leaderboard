...
- Percentiles are computed on the 1-decimal displayed values, so any scale change shifts a few ranks by exactly 1 through tie-breaking. That is not a rank change. Check for it rather than assuming an affine rescale leaves percentiles untouched.
...
- A combined `2TM`..`10TM` row has no park of its own, so `apply_era_plus` resolves it as the INNINGS-WEIGHTED mean of that pitcher's own MLB stint rows (`combined_park_map`). `park.get('2TM')` falls through to neutral, which scored 93 rows against a neutral park until 2026-08-19; the ex-Rockies arms were off by up to 0.52 ERA. ROC stints are excluded from the weighting.
