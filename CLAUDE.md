...
- A ROC/AAA row must never resolve a combined `2TM`/`3TM` row. `_combinedKey` groups on `mlbId`, so a ROC stint of a traded player shares a key with his MLB rows. Go through `Aggregator._combinedRowFor(map, row)` in `js/aggregator.js`, never a bare `combinedBy*[_combinedKey(row)]` read. `pipeline/compute.py:769` already routes ROC rows out before the same check.
