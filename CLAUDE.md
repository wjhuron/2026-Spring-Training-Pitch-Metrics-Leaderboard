...
- Expected movement (`pipeline/xmove.py`) is scored PER PITCH and the site and card only sum `sumXIVB`/`sumXHB` from the micro rows. The basis is nonlinear in release tilt, so scoring it at group means is wrong by up to 3" on gyro sliders; never put the model math back into `js/aggregator.js`. Its harmonic columns are near-collinear when a group's tilt barely varies (CH_L needed 2,000 pitches unpenalised), which is why the fit is a ridge: sweep `XMOVE_RIDGE` and `XMOVE_MIN_N` jointly, never one alone.
