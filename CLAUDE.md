...
- Date-window cards need a pool built first: `python3 scripts/builders/build_window_leaderboard.py START END [--cached]`. Without one the card still renders, but every percentile is grey and the + family reads `—`, because a percentile needs an all-MLB pool over the same window. `--cached` reuses `data/_window_pitches_cache.pkl` and skips the 2m30s Sheets read; drop it after Sheets is appended to.
